...
train_count = 0.8 * total_count
val_count = 0.1 * total_count
test_count = 0.1 * total_count
...
for epoch in range(nums_epoch):
    ...
    print(f"Epoch {epoch+1} / {nums_epoch}, Train MSE: {avg_train_loss:.4f}, Val MSE: {avg_val_loss:.4f}")

    """ If current val_loss is the lowest, save the model parameters """
    if avg_val_loss < best_val_loss:
        best_val_loss = avg_val_loss
        torch.save(model.state_dict(), "best_model.pth")    # model.state_dict(): returns a dict of all trainable parameters (e.g., weights and biases)
...
def main():
    ...
    for epoch in range(nums_epoch):
        ...
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            torch.save(model.state_dict(), f"best_model_{epoch + 1}.pth")    # model.state_dict(): returns a dict of all trainable parameters (e.g., weights and biases)
    
if __name__ == "__main__":
    main()
>>>>>>> 5ab5510 (First Complete!)
